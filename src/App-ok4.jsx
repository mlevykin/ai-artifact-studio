...
const T = {
  ...
  en: {
    ...
    model: "Model",
    ollamaUrl: "Ollama URL",
    updateModels: "Refresh model list",
    ...
    pngError: "Failed to convert SVG to PNG.\nTry exporting as SVG.",
    sending: "...",
    send: "→",
  },
};
...
Types:
- mermaid → ANY diagram: flowchart, sequence, class, state, ER, gantt, timeline. Content = raw mermaid syntax only, no backticks. In sequenceDiagram: "Note over" accepts max 2 participants (e.g. "Note over A,B" not "Note over A,B,C"). In timeline: format is "period : event" (period on LEFT, event on RIGHT), periods must be plain text labels not HH:MM times. In flowchart: every arrow MUST have a target node id (e.g. "A -->|label| B" not "A -->|label]"). Never use quoted strings as node ids in arrows. Never put double quotes inside edge labels "|text|".
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ position: "absolute", top: 10, right: 10, zIndex: 10, display: "flex", gap: 4, alignItems: "center" }}>
        ...
        <button onClick={() => setZoom(1)} style={zoomBtnStyle} title="Reset">⊙</button>
      </div>
      <div style={{ flex: 1, overflow: "auto", background: "#f0f0f0" }}>
        <div style={{ transformOrigin: "top left", transform: `scale(${zoom})`, width: `${100 / zoom}%`, minHeight: `${100 / zoom}%` }}>
          <iframe
            srcDoc={content}
            style={{ width: "100%", height: `${100 / zoom}vh`, border: "none", background: "#fff", display: "block" }}
  ...
  useEffect(() => {
    ...
    const cleanCode = (() => {
      let c = code
        ...
        .replace(/Note\s+over\s+(\w+),\w+,(\w+)/gi, "Note over $1,$2")
        .replace(/Note\s+over\s+(\w+)(?:,\w+){2,}/gi, (m) => {
          const parts = m.replace(/Note\s+over\s+/i, "").split(",");
          return `Note over ${parts[0]},${parts[parts.length - 1]}`;
        });

      // Фикс оборванных стрелок: -->|"текст"] без целевого узла → убираем такие строки
      c = c.replace(/^\s*\w+\s*-->\|[^|]*\]\s*$/gm, "");
      // Убираем кавычки внутри меток стрелок -->|"текст"| → -->|текст|
      c = c.replace(/-->\|"([^"]+)"\|/g, "-->|$1|");
      // Фикс узлов со строками в кавычках как источник стрелок: "текст" -.-> X → убираем
      c = c.replace(/^\s*"[^"]+"\s*-+[->]+.*$/gm, "");

      // Фикс timeline: "событие : HH:MM" → "HH:MM : событие"
...
function ArtifactPreview({ artifact, viewMode, streaming, onSvgReady, t }) {
  ...
  if (streaming) return (
    <div className="artifact-empty"><span className="spin">⟳</span> {t.generating}</div>
  );

  if (artifact.type === "mermaid") return (
    <div style={{ width: "100%", height: "100%" }}>
      <MermaidView code={artifact.content} onSvgReady={onSvgReady} t={t} />
    </div>
  );
...
export default function App() {
  ...
  const toggleLang = () => {
    ...
    setLang(next);
    localStorage.setItem("as_lang", next);
  };
  ...
  const sendMessage = useCallback(async () => {
    ...
    // Если открыт артефакт — добавляем его в контекст как "текущий артефакт для редактирования"
    const currentArt = artifact;
    const histMsgs = [...messages, userMsg].map(m => ({ role: m.role, content: m.text }));

    // Инжектируем контекст артефакта в последнее сообщение пользователя
    if (currentArt) {
