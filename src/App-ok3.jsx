...
Types:
- mermaid → ANY diagram: flowchart, sequence, class, state, ER, gantt. Content = raw mermaid syntax only, no backticks. In sequenceDiagram: "Note over" accepts max 2 participants (e.g. "Note over A,B" not "Note over A,B,C").
...
function MermaidView({ code, onSvgReady, t }) {
  ...
  useEffect(() => {
    ...
    const cleanCode = code
      ...
      // Note over с 3+ участниками → оставляем только первых двух
      .replace(/Note\s+over\s+(\w+),\w+,(\w+)/gi, "Note over $1,$2")
      // Note over с 4+ участниками (на всякий случай)
          ...
          : <>
              ...
              <div className="input-row">
                <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey}
                  rows={1} placeholder={t.inputPlaceholder} className="chat-input"
